...
public class GetWordsLynx {
    ...
    public static Hashtable<String, Integer> runLynx(String url) {
        ...
        try {
            ...
            String cmdline[] = {"/opt/local/bin", "--dump", url };
        ...
        Hashtable<String, Integer> document = new Hashtable<String, Integer>();
        StringTokenizer st = new StringTokenizer(output.toString());

        while (st.hasMoreTokens()) {
            String tok = st.nextToken();
            if(document.containsKey(tok)){
            	int i = document.get(tok);
            	document.put(tok,i+1);
            }
            else{
            	document.put(tok, 1);
            }
